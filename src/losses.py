...
def get_patch(x, x_start, y_start, patch_size):
    """
    이미지 패치를 추출하는 함수
        - 주어진 좌표(x_start, y_start)에서 시작하여 패치 크기(patch_size)만큼 이미지를 잘라냄
        - 이미지 시프트 연산에 사용
...
class Shift(nn.Module):
    """
    이미지 시프트 연산을 위한 비학습 컨볼루션 레이어
    미리 정의된 필터를 사용해서 이미지를 원하는 방향으로 시프트
    ShiftNet 대신 사용(ShiftNet은 학습 가능한 레이어)

    모델의 결과물(SR)과 정답(HR)의 손실을 계산하기 전에, HR을 조금씩 움직여가며(Shift) SR 이미지와 오차가 가장 작아지는 위치를 찾고,
    그 위치에서의 손실을 계산(정렬) -> 위치에 대한 페널티를 제거하여 구조 복원에만 집중하도록 함.
    """
