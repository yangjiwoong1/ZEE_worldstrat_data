...
from src.modules import SRCNN as srcnn
...
def generate_model_backbone(args, dataloaders):
    ...
    mask_channels = dataloaders["train"].dataset[0]["lrc"].shape[-3]
    if args.use_masks:
...
def load_dataset(args):
    ...
    if args.dataset == "JIF":
        ...
        raise Exception("Undefined dataset. Only 'JIF' is supported in this training pipeline.")
...
def add_list_of_aois_to_args(args):
    ...
    if args.list_of_aois is not None:
        # Keep as DataFrame with 'split' column so downstream dataset builder
        # can generate train/val/test splits correctly.
        args.list_of_aois = pd.read_csv(args.list_of_aois)
    return args
