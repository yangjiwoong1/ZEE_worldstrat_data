...
class LitModel(pl.LightningModule):
    ...
    def loss(self, batch, metrics, baseline_metrics, prefix):
        ...
        # 4. 모델 통과 (내부에서는 [B, R, C, H, W] 형태로 처리)
        y_hat = self(x, y)
        
        # 5. Loss 계산을 위해 첫 번째 프레임만 선택
        y = y[:, 0]      # [B, R, C, H, W] -> [B, C, H, W]
        y_hat = y_hat[:, 0]  # [B, R, C, H, W] -> [B, C, H, W]
        
        # 6. 밝기 보정 ([B, C, H, W] 형태로)
        y_hat = self.bias_adjust(y, y_hat)
        
        # 7. 메트릭 계산 ([B, C, H, W] 형태로)
        m, mse, mae, ssim, tv = self.compute_metrics(y, y_hat, metrics, prefix)
        
        # 8. baseline도 동일하게 처리
        y_hat_base = self.baseline(x)[:, 0]  # 첫 번째 프레임만
    ...
    def apply_masks_to_x(self, x, batch):
        ...
        if self.hparams.use_masks:
            masks = batch["lrc"]
            if x.ndim == 5 and masks.ndim == 4: masks = masks[:, None, ...]
            if x.ndim == 4 and masks.ndim == 5: masks = masks[:, 0, ...]
